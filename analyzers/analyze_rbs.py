...
def analyze_rbs():
    '''Function that analyzes the RB data and creates a model from it.'''
    players = {}
    teams = {}
    opponents = {}
    ...
    with io.open(os.path.join(path_files, 'computed/rb_data.csv'), 'w', newline='') as stat_file:
    ...
    df = pandas.read_csv(os.path.join(path_files, 'computed/rb_data.csv'), delimiter=",")
    ...
    for label in labels:
        ...
        pickle.dump(regr, open(os.path.join(path_files, "models/rb_" + label + ".pkl"), "wb"))
